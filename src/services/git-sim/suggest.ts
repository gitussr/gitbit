...
import { aheadBehind, contains, currentBranch, isClean, stagedChanges, unstagedChanges, untrackedFiles, headCommitId } from './repo'
...
export function suggest(state: RepoState): string[] {
  ...
  // With a remote, what matters next is how far this branch is from it.
  const head = headCommitId(state)
  if (state.remote && head) {
    const tracking = state.upstreams[branch]
    if (!tracking) return [`git push -u ${state.remote.name} ${branch}`, 'git remote -v', 'git log --oneline']
    const tip = state.remoteBranches[tracking]
    const { ahead, behind } = tip ? aheadBehind(state, head, tip) : { ahead: 1, behind: 0 }
    if (behind > 0) return ['git pull', 'git status', 'git log --oneline']
    if (ahead > 0) return ['git push', 'git status', 'git log --oneline']
    return ['git fetch', 'git status', 'git log --oneline']
  }
